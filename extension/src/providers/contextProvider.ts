            ...
            try {
                ...
                return entries.map((entry) => {
                    ...
                    return {
                        ...
                        label: `Context Forge: ${entry.kind} (${new Date(timestampMs).toLocaleTimeString()})`,
                        modelDescription: `Preserved context from Context Forge (kind: ${entry.kind}, id: ${entry.id}, saved at ${new Date(timestampMs).toISOString()}). This context was saved before compaction and should be treated as important prior knowledge.`,
                        value: entry.content,
                    };
                });
            } catch (err: unknown) {
