...
#[test]
fn scoped_save_and_query_do_not_cross_contaminate() {
    let config = Config::default();
    let cf = ContextForge::open(config).expect("open in-memory store");
    ...
    let global_hits = cf.query("deploy notes", None, 1000).expect("global query");
    assert_eq!(global_hits.len(), 2);

    // clear_scope(A) removes only A's entries.
    let cleared = cf.clear_scope("project:alpha").expect("clear scope A");
    assert_eq!(cleared, 1);
    ...
    assert_eq!(cf.count().expect("count"), 1);
}

#[test]
fn match_all_query_respects_scope() {
    let config = Config::default();
    let cf = ContextForge::open(config).expect("open in-memory store");
